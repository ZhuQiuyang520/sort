...
public class BuildEditScript : MonoBehaviour
{
    ...
    public static void Check(int buildTargetIndex, BuildInfoData info)
    {
        ...
        if (Resources.FindObjectsOfTypeAll<RatLadyTen>()[0].TurnWan != info.BaseUrl)
        {
            isSame = false;
            Debug.Log("域名");
        }
        ...
        if (Resources.FindObjectsOfTypeAll<ADImagery>()[0].MAX_SDK_KEY != info.Applovin_SDK_KEY)
        {
            isSame = false;
            Debug.Log("admanager key");
        }
        if (Resources.FindObjectsOfTypeAll<ADImagery>()[0].MAX_REWARD_ID != info.Applovin_REWARD_ID)
        {
            isSame = false;
            Debug.Log("reward id");
        }
        if (Resources.FindObjectsOfTypeAll<ADImagery>()[0].MAX_INTER_ID != info.Applovin_INTER_ID)
        {
            isSame = false;
            Debug.Log("inter id");
        }
        if (Resources.FindObjectsOfTypeAll<RatLadyTen>()[0].ZealSolo != info.GameCode)
        {
            isSame = false;
            Debug.Log("gamecode");
        }
    ...
    public static void SuperBuild(int buildTargetIndex, BuildInfoData info)
    {
        ...
        Resources.FindObjectsOfTypeAll<ADImagery>()[0].MAX_SDK_KEY = info.Applovin_SDK_KEY;
        Resources.FindObjectsOfTypeAll<ADImagery>()[0].MAX_REWARD_ID = info.Applovin_REWARD_ID;
        Resources.FindObjectsOfTypeAll<ADImagery>()[0].MAX_INTER_ID = info.Applovin_INTER_ID;
        ...
        if (!GameObject.Find("MainManager") || !GameObject.Find("MainManager").GetComponent<FecundTactImagery>())
        {
            if (GameObject.Find("MainManager"))
            {
                GameObject.Find("MainManager").AddComponent<FecundTactImagery>();
            }
            else
            {
                GameObject mainObj = new GameObject("MainManager");
                mainObj.AddComponent<FecundTactImagery>();
            }
        }
        GameObject.Find("MainManager").GetComponent<FecundTactImagery>().VirginID = info.Adjust_APP_ID;

        if (!GameObject.Find("MainManager").GetComponent<LuckIDImagery>())
        {
            GameObject.Find("MainManager").AddComponent<LuckIDImagery>();
        }

#if UNITY_IOS
        GameObject.Find("MainManager").GetComponent<LuckIDImagery>().appid = info.Rate_ID;
#endif
#if UNITY_ANDROID
        GameObject.Find("MainManager").GetComponent<LuckIDImagery>().appid = info.PackageName;
#endif
        ...
        Resources.FindObjectsOfTypeAll<RatLadyTen>()[0].ZealSolo = info.GameCode;
        Resources.FindObjectsOfTypeAll<RatLadyTen>()[0].TurnWan = info.BaseUrl;
        Resources.FindObjectsOfTypeAll<RatLadyTen>()[0].TurnGhostWan = info.BaseUrl + CExcite.GhostWan;
        Resources.FindObjectsOfTypeAll<RatLadyTen>()[0].TurnExciteWan = info.BaseUrl + CExcite.ExciteWan;
        Resources.FindObjectsOfTypeAll<RatLadyTen>()[0].TurnWakeWan = info.BaseUrl + CExcite.WakeWan;
        Resources.FindObjectsOfTypeAll<RatLadyTen>()[0].TurnFecundWan = info.BaseUrl + CExcite.AdjustWan;
        EditorSceneManager.SaveScene(EditorSceneManager.GetActiveScene());

#if UNITY_IOS
        string url = info.BaseUrl + CExcite.ExciteWan + info.GameCode + "&channel=" + "AppStore" + "&version=" + buildWindow.Version;
#elif UNITY_ANDROID
        string url = info.BaseUrl + CExcite.ConfigUrl + info.GameCode + "&channel=" + "GooglePlay" + "&version=" + buildWindow.Version;
#else
        string url = info.BaseUrl + CExcite.ConfigUrl + info.GameCode + "&channel=" + "GooglePlay" + "&version=" + buildWindow.Version;
#endif
