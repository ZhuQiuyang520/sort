...
{
    public static void Fetus<TKey, TValue>(this IDictionary<TKey, TValue> first, IDictionary<TKey, TValue> second)
    {
    ...
    public class PutSampleShow : BeerZoologist<PutSampleShow>
{
    // Start is called before the first frame update
    public bool FuseEmblem;
    public bool DyGrit;

    static public PutSampleShow instance;
    ...
    /// <returns></returns>
    public Vector3 RobStudyInput(GameObject gameobj)
    {
    ...
    /// <returns></returns>
    public GameObject PutYucatanZealBranch(Transform SelfTransform,List<Transform> listTemp)
    {
    ...
    /// <param name="p2"></param>
    /// <returns></returns>
    public float InputUpRidge(Vector2 p1, Vector2 p2)
    {
        Vector2 p;
        p.x = p2.x - p1.x;
        p.y = p2.y - p1.y;
    ...
    /// <returns></returns>
    public float Ridge180Up360(float angle)
    {
    ...
    /// <param name="p2"></param>
    /// <returns></returns>
    public int PutBelleGrassland(Vector2 p1, Vector2 p2)
    {
        float angle = Ridge180Up360(InputUpRidge(p1, p2));
    ...
    /// <returns></returns>
    public Vector2 RobHarmfulExamGibe(Renderer Renderer)
    {
    ...
    public Vector2 RobHarmfulExamGibe(Image image)
    {
    ...
    /// <param name="obj"></param>
    /// <returns></returns>
    public Vector2 RobHeraldGibe(GameObject obj)
    {
    ...
    /// </summary>
    /// <returns></returns>
    public Vector2 RobSouthEsoteric()
    {
        
        Vector3 v = Input.mousePosition;
        float xa = v.x / Screen.width;
        float ya = v.y / Screen.height;
        float screenW = RobTenantAnvil();
        float screenH = RobTenantMotive();
        float x = -screenW/2 + xa * screenW;
        float y = -screenH/2 + ya * screenH;
        return new Vector2(x, y);   
    }

    public int RobEntombGourd(List<string> weightList)
    {
        float sumWeight = 0;
    ...
    public int[] PutVerifyStrategy(int total, int count)
    {
    ...
    public Dictionary<string, object> MeadowUpHomeotherm(string value)
    {
        if (value.Length < 1)
        {
            return null;
    ...
    public string HomeothermUpMeadow(Dictionary<string, object> Info)
    {
    ...
    public int DayTunaBoth(string dateStart, DateTime dateEnd)
    {
    ...
    public int PigTunaBoth(string dateStart, DateTime dateEnd)
    {
    ...
    public TimeSpan TunaBoth(DateTime dateStart, DateTime dateEnd)
    {
    ...
    public Vector3 PutMatAtHand(Canvas canvas, Text text, string strFragment)
    {
        int strFragmentIndex = text.text.IndexOf(strFragment);//-1表示不包含strFragment
        Vector3 stringPos = Vector3.zero;
        if (strFragmentIndex > -1)
        {
            Vector3 firstPos = PutMatAtHand(canvas, text, strFragmentIndex);
            Vector3 lastPos = PutMatAtHand(canvas, text, strFragmentIndex + strFragment.Length);
            stringPos = (firstPos + lastPos) * 0.5f;
        }
        else
        {
            stringPos = PutMatAtHand(canvas, text, strFragmentIndex);
        }
        return stringPos;
    }

    public Vector3 PutMatAtHand(Canvas canvas, Text text, int charIndex)
    {
    ...
    /// <returns></returns>
    public float ChicopeeByBone(Vector3 pB, Vector3 pA1, Vector3 pA2)
    {
    ...
    private static byte[] Sour= { 0x12, 0x34, 0x56, 0x78, 0x90, 0xAB, 0xCD, 0xEF };
    /// <summary>
    /// DES加密字符串
    /// </summary>
    ...
    public static string CalorieDES(string encryptString, string key = "abcdefgh")
    {
        try
        {
    ...
    public static  string ShelterDES(string decryptString, string key = "13717421")
    {
        byte[] rgbKey = Encoding.UTF8.GetBytes(Application.identifier.Substring(0, 8));
        byte[] rgbIV = Sour;
