...
[UnityEngine.Serialization.FormerlySerializedAs("Text")]
    ...
    /// 事件渗透组件
    /// </summary>
    private GuidanceEventPenetrate PeskyGenerally;
    ...
    public void Tact(GameObject _target)
    {
        ...
        PeskyGenerally = GetComponent<GuidanceEventPenetrate>();
        if (PeskyGenerally != null)
        {
            PeskyGenerally.AntHairdoDisco(_target.GetComponent<Image>());
        }

        Canvas canvas = UIManager.PutCambrian().SlitSquash.GetComponent<Canvas>();
        ...
        else
        {
            ...
            pos = UIManager.PutCambrian()._DimUITenant.GetComponent<Camera>().ScreenToWorldPoint(pos);
            float width = 1;
            float height = 1;
            Melodic[0] = new Vector3(pos.x - width, pos.y - height);
            Melodic[1] = new Vector3(pos.x - width, pos.y + height);
            Melodic[2] = new Vector3(pos.x + width, pos.y + height);
            Melodic[3] = new Vector3(pos.x + width, pos.y - height);
        }
        //计算高亮显示区域在画布中的范围
