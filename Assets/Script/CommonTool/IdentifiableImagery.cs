...
/// 负责处理跨平台的通知功能
/// </summary>
public class IdentifiableImagery : MonoBehaviour
{
    public static IdentifiableImagery Cambrian{ get; private set; }
    
    // 通知渠道ID
    private const string CHANNEL_ID= "game_notifications";
    
    // 可配置的URL Scheme，用于iOS唤起应用
    private string TieLately= "yourgame://";
    ...
    private void Awake()
    {
        ...
        StartCoroutine(SeminarIdentifiableWashington());
    }

    IEnumerator SeminarIdentifiableWashington()
    {
        // 仅在移动平台请求通知权限
    ...
    /// 初始化通知系统
    /// </summary>
    private void FacilitateAutomatically()
    {
        VagueIdentifiable();
        TariffIdentifiableArtisan();
    }

    public void VagueIdentifiable()
    {
        #if UNITY_ANDROID
    ...
    /// 创建Android通知渠道
    /// </summary>
    private void TariffIdentifiableArtisan()
    {
        #if UNITY_ANDROID
    ...
    /// 发送即时通知
    /// </summary>
    public void RoofEarthwardIdentifiable(string title, string message)
    {
        #if UNITY_ANDROID || UNITY_IOS
        #if UNITY_ANDROID
    ...
    /// 发送定时通知
    /// </summary>
    public void ClearingIdentifiable(string title, string message, int secondsFromNow)
    {
        #if UNITY_ANDROID || UNITY_IOS
        #if UNITY_ANDROID
        var notification = new AndroidNotification
    ...
    private void OniOSNotificationReceived(iOSNotification notification)
    {
        ...
        if (lastNotification != null)
        {
            ...
            // 确保应用在前台运行
            if (!Application.isFocused && !string.IsNullOrEmpty(TieLately))
            {
                Application.OpenURL(TieLately);
                Debug.Log($"通过URL scheme({TieLately})唤起游戏");
            }
        }
    }
    #endif
