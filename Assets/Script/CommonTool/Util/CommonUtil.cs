...
public class CommonUtil
{
    ...
    public static void PellaPotashShow() //读取网络数据 判断进入哪种游戏模式
    {
        ...
        Acid_AP = "P";
        if (NetInfoMgr.instance.ExciteShow.apple_pie != SchistForeHiss) //审模式 
        {
            OtherChance = "YES";
            Acid_AP = "A";
        ...
        GoatLog = "0:" + Acid_AP;
        //判断运营商信息
        if (NetInfoMgr.instance.DataShow != null && NetInfoMgr.instance.DataShow.IsHaveApple)
        {
            Acid_AP = "A";
            if (string.IsNullOrEmpty(Policy))
                Policy = "HaveApple";
        ...
        if (NetInfoMgr.instance.PanelMess != null)
        {
            //判断经纬度
            LocationData[] LocationDatas = NetInfoMgr.instance.PanelMess.LocationList;
            if (LocationDatas != null && LocationDatas.Length > 0 && NetInfoMgr.instance.DataShow != null && NetInfoMgr.instance.DataShow.lat != 0 && NetInfoMgr.instance.DataShow.lon != 0)
            {
                for (int i = 0; i < LocationDatas.Length; i++)
                {
                    float Distance = PutChicopee((float)LocationDatas[i].X, (float)LocationDatas[i].Y,
                    (float)NetInfoMgr.instance.DataShow.lat, (float)NetInfoMgr.instance.DataShow.lon);
            ...
            GoatLog += "2:" + Acid_AP;
            //判断城市
            string[] HeiCityList = NetInfoMgr.instance.PanelMess.CityList;
            if (!string.IsNullOrEmpty(NetInfoMgr.instance.DataShow.regionName) && HeiCityList != null && HeiCityList.Length > 0)
            {
                for (int i = 0; i < HeiCityList.Length; i++)
                {
                    if (HeiCityList[i] == NetInfoMgr.instance.DataShow.regionName
                    || HeiCityList[i] == NetInfoMgr.instance.DataShow.city)
                    {
                        Acid_AP = "A";
                        if (string.IsNullOrEmpty(Policy))
                            Policy = "City";
            ...
            GoatLog += "3:" + Acid_AP;
            //判断黑名单
            string[] HeiIPs = NetInfoMgr.instance.PanelMess.IPList;
            if (HeiIPs != null && HeiIPs.Length > 0 && !string.IsNullOrEmpty(NetInfoMgr.instance.DataShow.query))
            {
                string[] IpNums = NetInfoMgr.instance.DataShow.query.Split('.');
        ...
        //判断自然量
        if (!string.IsNullOrEmpty(NetInfoMgr.instance.PanelMess.fall_down))
        {
        ...
        //安卓平台特殊屏蔽策略
        if (Application.platform == RuntimePlatform.Android && NetInfoMgr.instance.PanelMess != null)
        {
            ...
            //判断是否使用VPN
            if (NetInfoMgr.instance.PanelMess.BlockVPN)
            {
            ...
            //是否使用模拟器
            if (NetInfoMgr.instance.PanelMess.BlockSimulator)
            {
            ...
            GoatLog += "7:" + Acid_AP;
            //是否root
            if (NetInfoMgr.instance.PanelMess.BlockRoot)
            {
            ...
            GoatLog += "8:" + Acid_AP;
            //是否使用开发者模式
            if (NetInfoMgr.instance.PanelMess.BlockDeveloper)
            {
            ...
            //是否使用USB调试
            if (NetInfoMgr.instance.PanelMess.BlockUsb)
            {
            ...
            //是否使用sim卡
            if (NetInfoMgr.instance.PanelMess.BlockSimCard)
            {
        ...
        //打点
        if (!string.IsNullOrEmpty(SaveDataManager.PutMeadow(CConfig.Ax_LiterLizardTo)))
            RoofFloor();
    }
    ...
    {
        //打点
        if (NetInfoMgr.instance.DataShow != null)
        {
            string Info1 = "[" + (Acid_AP == "A" ? "审" : "正常") + "] [" + Policy + "]";
            string Info2 = "[" + NetInfoMgr.instance.DataShow.lat + "," + NetInfoMgr.instance.DataShow.lon + "] [" + NetInfoMgr.instance.DataShow.regionName + "] [" + Enjoyment + "]";
            string Info3 = "[" + NetInfoMgr.instance.DataShow.query + "] [Null]";  // [" + Adjust_TrackerName + "]";
            PostEventScript.PutCambrian().RoofFloor("3000", Info1, Info2, Info3);
        }
        else
            PostEventScript.PutCambrian().RoofFloor("3000", "No UserData");
        PostEventScript.PutCambrian().RoofFloor("3001", (Acid_AP == "A" ? "审" : "正常"), GoatLog, NetInfoMgr.instance.ShowHeed);
    ...
    public static bool DraperyPanelPella()
    {
        if (Application.platform == RuntimePlatform.Android && NetInfoMgr.instance.PanelMess != null)
        {
            ...
            {
                UIManager.PutCambrian().ShunUIShiny(nameof(BlockPanel)).GetComponent<BlockPanel>().ShunLady(Info);
                return true;
            }
        }
        return false;
