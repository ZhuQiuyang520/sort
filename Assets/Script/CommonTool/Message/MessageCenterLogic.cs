...
public class MessageCenterLogic:MonoSingleton<MessageCenterLogic>
{
    ...
    private Dictionary<string, Action<MessageData>> LeafcutterPottery;
    ...
    {
        //初始化消息字典
        LeafcutterPottery = new Dictionary<string, Action<MessageData>>();
    }
    ...
    public void Eusocial(string key, Action<MessageData> action)
    {
    ...
    /// <param name="key">消息名</param>
    /// <param name="action">消息事件</param>
    public void Slowly(string key, Action<MessageData> action)
    {
    ...
    public void Roof(string key, MessageData data = null)
    {
        if (LeafcutterPottery.ContainsKey(key) && LeafcutterPottery[key] != null)
        {
