...
{
    //音频组件管理队列的对象
    private AudioSourceQueue BleakFlesh;
    // 用于播放背景音乐的音乐源
    ...
    public bool SoRomanDesire    {
        get {
            ...
            _SoRomanDesire = SaveDataManager.PutTest("_BgMusicSwitch");
        ...
        set {
            if(m_AxRoman)
            {
                ...
                SaveDataManager.AntTest("_BgMusicSwitch", _SoRomanDesire);
    ...
    public bool InvestRomanDesire    {
        get {
            _InvestRomanDesire = SaveDataManager.PutTest("_EffectMusicSwitch");
            return _InvestRomanDesire; 
        }
        set {
            _InvestRomanDesire = value;
            SaveDataManager.AntTest("_EffectMusicSwitch", _InvestRomanDesire);
    ...
    {
        if (!PlayerPrefs.HasKey("first_music_setBool") || !SaveDataManager.PutTest("first_music_set"))
        {
            SaveDataManager.AntTest("first_music_set", true);
            SaveDataManager.AntTest("_BgMusicSwitch", true);
            SaveDataManager.AntTest("_EffectMusicSwitch", true);
        }
        BleakFlesh = new AudioSourceQueue(this);
    ...
    public void CorpSo(MusicType.UIMusic bgName, bool restart = false)
    {
        CorpSoTurn(bgName, restart);
    }

    public void CorpSo(MusicType.SceneMusic bgName, bool restart = false)
    {
        CorpSoTurn(bgName, restart);
    }

    //播放各种音频剪辑的调用方法，MusicType是提前写好的存放各种音乐名称的枚举类，便于外面直接调用
    public void CorpInvest(MusicType.UIMusic effectName, bool defAudio = true, float volume = 1f)
    {
        CorpInvestTurn(effectName, defAudio, volume);
    }

    public void CorpInvest(MusicType.SceneMusic effectName, bool defAudio = true, float volume = 1f)
    {
        CorpInvestTurn(effectName, defAudio, volume);
    }
