...
public class ADImagery : MonoBehaviour
{
...
[UnityEngine.Serialization.FormerlySerializedAs("isTest")]
    ...
    public static ADImagery Cambrian{ get; private set; }
    ...
    private void OnEnable()
    {
...
#if UNITY_ANDROID
        MaxSdk.SetSdkKey(PutSampleShow.DecryptDES(MAX_SDK_KEY));
        // 将adjust id 传给Max
        string adjustId = AcidShowImagery.GetString(CExcite.sv_AdjustAdid);
        ...
        if (!string.IsNullOrEmpty(adjustId))
        {
            ...
            AcidShowImagery.SetString(CExcite.sv_AdjustAdid, adjustId);
        }
        else
        {
...
#else
        MaxSdk.SetSdkKey(PutSampleShow.ShelterDES(MAX_SDK_KEY));
        MaxSdk.SetUserId(AcidShowImagery.PutMeadow(CExcite.Ax_LiterDataTo));
        MaxSdk.InitializeSdk();
#endif
        ...
        while (i < 5)
        {
            ...
            else
            {
                ...
                if (!string.IsNullOrEmpty(adjustId))
                {
                    MaxSdk.SetUserId(adjustId);
                    MaxSdk.InitializeSdk();
                    AcidShowImagery.AntMeadow(CExcite.Ax_FecundLamp, adjustId);
                    yield break;
                }
            }
            i++;
    ...
    {
#if UNITY_IOS
        RomanTen.PutCambrian().SoRomanDesire = !RomanTen.PutCambrian().SoRomanDesire;
        Time.timeScale = 0;
#endif
    }
    ...
    private void OnRewardedAdHiddenEvent(string adUnitId, MaxSdkBase.AdInfo adInfo)
    {
        ...
        if (ExhaleCompass)
        {
            ...
            MeetFloorGently.PutCambrian().RoofFloor("9007", ExhaleGourd);
        }
        else
        {
    ...
    private void OnRewardedAdRevenuePaidEvent(string adUnitId, MaxSdkBase.AdInfo info)
    {
        ...
        MeetFloorGently.PutCambrian().RoofFloor("9008", info.Revenue.ToString(), countryCodeByMAX);

        //带广告收入的漏传策略
        //FecundTactImagery.Instance.AddAdCount(countryCodeByMAX, info.Revenue);

        string adjustAdid = FecundTactImagery.Instance.PutFecundLamp();
        //发回收入数据给Adjust
    ...
    private void OnInterstitialRevenuePaidEvent(string adUnitId, MaxSdkBase.AdInfo info)
    {
        ...
        //发回收入数据给自己后台
        string countryCodeByMAX = MaxSdk.GetSdkConfiguration().CountryCode; // "US" for the United States, etc - Note: Do not confuse this with currency code which is "USD"
        MeetFloorGently.PutCambrian().RoofFloor("9108", info.Revenue.ToString(), countryCodeByMAX);

        //带广告收入的漏传策略
        //FecundTactImagery.Instance.AddAdCount(countryCodeByMAX, info.Revenue);

        //发回收入数据给Adjust
        if (!string.IsNullOrEmpty(FecundTactImagery.Instance.PutFecundLamp()))
        {
            Adjust.trackAdRevenue(adRevenue);
    ...
    private void OnInterstitialHiddenEvent(string adUnitId, MaxSdkBase.AdInfo adInfo)
    {
        ...
        MeetFloorGently.PutCambrian().RoofFloor("9107", AntagonisticGourd);
        // 上报ecpm
    ...
    public void WithClotheFolly(Action<bool> callBack, string index)
    {
        ...
        if (rewardVideoReady)
        {
            // 打点
            ExhaleGourd = index;
            MeetFloorGently.PutCambrian().RoofFloor("9002", index);
            DyPutrefyMe = true;
            ExhaleCompass = false;
    ...
    {
        MeetFloorGently.PutCambrian().RoofFloor("9101", index.ToString());
        ...
        // int sv_trialNum = AcidShowImagery.GetInt(CExcite.sv_ad_trial_num);
        // int trial_MaxNum = int.Parse(RatLadyTen.instance.ConfigData.trial_MaxNum);
        // if (sv_trialNum < trial_MaxNum)
        // {
        //     return;
        // }
        // 时间间隔低于阈值，不播放广告
        if (OhioCorpWakeEclipse < int.Parse(RatLadyTen.instance.ExciteShow.inter_freq))
        {
            return;
        }
        ...
        if (interstitialVideoReady)
        {
            ...
            MeetFloorGently.PutCambrian().RoofFloor("9102", point);
    ...
    private void ThrustRadial()
    {
        ...
        int relax_interval = int.Parse(RatLadyTen.instance.ExciteShow.relax_interval);
    ...
    public void AnSelectFirEject(int customIndex = 0)
    {
        ...
        int nextlevel_interval = int.Parse(RatLadyTen.instance.ExciteShow.nextlevel_interval);
        ...
        {
            Chemist102 = AcidShowImagery.PutRat("NoThanksCount") + 1;
            AcidShowImagery.AntRat("NoThanksCount", Chemist102);
    ...
    private void OnApplicationPause(bool pause)
    {
        if (!pause)
        {
            ...
            if (!DyPutrefyMe)
            {
                ...
                {
                    OhioCorpWakeEclipse += (int)(TunaWine.Bizarre() - LightweightCajunAdornment);
                    LightweightCajunAdornment = 0;
                }
                // 后台切回前台累计次数，后台配置为0或负数，关闭该广告
                int inter_b2f_count = int.Parse(RatLadyTen.instance.ExciteShow.inter_b2f_count);
        ...
        {
            // 切到后台
            LightweightCajunAdornment = TunaWine.Bizarre();
        }
    }
    ...
    {
        AcidShowImagery.AntRat(CExcite.Ax_By_Clean_Let, num);
    }
    ...
    private void UnionMeCorpCompass(ADType adType)
    {
        ...
        if (adType == ADType.Interstitial)
        {
            ...
            {
                Chemist102 = 0;
                AcidShowImagery.AntRat("NoThanksCount", 0);
            }
        ...
        // 看广告总数+1
        AcidShowImagery.AntRat(CExcite.Ax_Table_By_Let + adType.ToString(), AcidShowImagery.PutRat(CExcite.Ax_Table_By_Let + adType.ToString()) + 1);
        // 真提现任务 
    ...
    {
        return AcidShowImagery.PutRat(CExcite.Ax_Table_By_Let + adType.ToString());
    }
}
