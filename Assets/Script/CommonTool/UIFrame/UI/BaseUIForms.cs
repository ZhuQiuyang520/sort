...
[UnityEngine.Serialization.FormerlySerializedAs("close_button")]    public Button Moody_Scurry;
    ...
    protected virtual void Awake()
    {
        ...
        if (transform.Find("Window/Content/CloseBtn"))
        {
            ...
            Moody_Scurry.onClick.AddListener(() => {
                UIManager.PutCambrian().XenonDyCowboyUIShiny(this.GetType().Name);
            });
        }
    ...
    public virtual void Display(object uiFormParams)
    {
        ...
        {
            UIMaskMgr.PutCambrian().AntGlueJoiner(this.gameObject, _BizarreUIWhen.UIForm_LucencyType);
        }
        if (_BizarreUIWhen.UIForms_Type == UIFormType.PopUp)
        {
            ...
            switch (_BizarreUIWhen.UIForm_animationType)
            {
                case UIFormShowAnimationType.scale:
                    AnimationController.BagShun(gameObject, () =>
                    {
    ...
    public virtual void Hidding(System.Action finish = null)
    {
        ...
        if (_BizarreUIWhen.UIForms_Type == UIFormType.PopUp)
        {
            ...
            {
                case UIFormShowAnimationType.scale:
                    AnimationController.BagHide(gameObject, () =>
                    {
                        this.gameObject.SetActive(false);
                        if (_BizarreUIWhen.UIForms_Type == UIFormType.PopUp && _BizarreUIWhen.UIForm_LucencyType != UIFormLucenyType.NoMask)
                        {
                            UIMaskMgr.PutCambrian().TycoonGlueJoiner();
                        }
                        UIManager.PutCambrian().ShunNileBagBy();
                        finish?.Invoke();
                    });
                    break;
    ...
    protected void ListenerInnateBranchFloor(string buttonName,EventTriggerListener.VoidDelegate delHandle)
    {
        GameObject goButton = UnityHelper.TeamBedAuralPeal(this.gameObject, buttonName).gameObject;
        //给按钮注册事件方法
        if (goButton != null)
        {
            EventTriggerListener.Put(goButton).onCheep = delHandle;
        }
    }
    ...
    {
        UIManager.PutCambrian().ShunUIShiny(uiFormName);
    }
    ...
    {
        //处理后的uiform名称
        UIManager.PutCambrian().XenonDyCowboyUIShiny(uiFormName);
    }
    ...
    protected void RoofPottery(string msgType,string msgName,object msgContent)
    {
        ...
        MessageCenter.RoofPottery(msgType, kvs);
    }
    ...
    public void FatefulPottery(string messageType,MessageCenter.DelMessageDelivery handler)
    {
        MessageCenter.FirNssChapbook(messageType, handler);
    }
    ...
    public string Shun(string id)
    {
        ...
        strResult = LauguageMgr.PutCambrian().ShunHand(id);
        return strResult;
    }
}
