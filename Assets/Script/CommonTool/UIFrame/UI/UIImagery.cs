...
/// </summary>
public class UIImagery : MonoBehaviour
{
    [HideInInspector]
[UnityEngine.Serialization.FormerlySerializedAs("MainCanvas")]    public Canvas SlitSquash;
    private static UIImagery _Cambrian= null;
    //ui窗体预设路径（参数1，窗体预设名称，2，表示窗体预设路径）
    private Dictionary<string, string> _WaxShinyAloof;
    //缓存所有的ui窗体
    private Dictionary<string, TurnUIShiny> _WaxALLUIShiny;
    //栈结构标识当前ui窗体的集合
    private Stack<TurnUIShiny> _BedBizarreUIShiny;
    //当前显示的ui窗体
    private Dictionary<string, TurnUIShiny> _WaxBizarreShunUIShiny;
    //临时关闭窗口
...
[UnityEngine.Serialization.FormerlySerializedAs("PanelName")]    public string PinonHiss;
    ...
    //得到实例
    public static UIImagery PutCambrian()
    {
        if (_Cambrian == null)
        {
            _Cambrian = new GameObject("_UIManager").AddComponent<UIImagery>();
    ...
    public void Awake()
    {
        ...
        //字段初始化
        _WaxALLUIShiny = new Dictionary<string, TurnUIShiny>();
        _WaxBizarreShunUIShiny = new Dictionary<string, TurnUIShiny>();
        _WaitUIShiny = new List<UIFormParams>();
        _WaxShinyAloof = new Dictionary<string, string>();
        _BedBizarreUIShiny = new Stack<TurnUIShiny>();
        //初始化加载（根ui窗体）canvas预设
        TactHangSquashConsume();
        //得到UI根节点，全屏节点，固定节点，弹出节点
        //Debug.Log("this.gameobject" + this.gameObject.name);
        _DimSquashSupersede = GameObject.FindGameObjectWithTag(SysPurely.SYS_TAG_CANVAS).transform;
        _DimSchist = BelleQuasar.TeamBedAuralPeal(_DimSquashSupersede.gameObject,SysPurely.SYS_NORMAL_NODE);
        _DimGoose = BelleQuasar.TeamBedAuralPeal(_DimSquashSupersede.gameObject, SysPurely.SYS_FIXED_NODE);
        _DimBagBy = BelleQuasar.TeamBedAuralPeal(_DimSquashSupersede.gameObject,SysPurely.SYS_POPUP_NODE);
        _Web = BelleQuasar.TeamBedAuralPeal(_DimSquashSupersede.gameObject, SysPurely.SYS_TOP_NODE);
        _DimUIWealthy = BelleQuasar.TeamBedAuralPeal(_DimSquashSupersede.gameObject,SysPurely.SYS_SCRIPTMANAGER_NODE);
        _DimUITenant = BelleQuasar.TeamBedAuralPeal(_DimSquashSupersede.gameObject, SysPurely.SYS_UICAMERA_NODE);
        //把本脚本作为“根ui窗体”的子节点
        BelleQuasar.FirAuralPealUpStickyPeal(_DimUIWealthy, this.gameObject.transform);
        //根UI窗体在场景转换的时候，不允许销毁
    ...
    {
        SlitSquash = UtteranceTen.PutCambrian().PartFever(SysPurely.SYS_PATH_CANVAS, false).GetComponent<Canvas>();
    }
    /// <summary>
    /// 显示ui窗体
    ...
    public GameObject ShunUIShiny(string uiFormName, object uiFormParams = null)
    {
        ...
        //ui窗体的基类
        TurnUIShiny baseUIForms = PartShinyUpALLUIShinyForce(uiFormName);
    ...
    public void VagueAcidUIShiny()
    {
     ...
     /// 根据UI窗体的名称，加载到“所有UI窗体”缓存集合中
      ...
      /// <returns></returns>
    private TurnUIShiny PartShinyUpALLUIShinyForce(string uiFormName)
    {
        //加载的返回ui窗体基类
        TurnUIShiny baseUIResult = null;
    ...
    {
        //ui窗体基类
        TurnUIShiny baseUIForm;
        //从所有窗体集合中得到的窗体
        TurnUIShiny baseUIFormFromAllCache;
    ...
    private void AprilUIMentalUpStandPileApply(string strUIFormsName, object uiFormParams)
    {
        ...
        TurnUIShiny baseUIForms;
        //所有窗体集合中的窗体基类
        TurnUIShiny baseUIFormsFromAllCache;
        _WaxBizarreShunUIShiny.TryGetValue(strUIFormsName, out baseUIForms);
        //正在显示ui窗体缓存集合里有记录，直接返回
        if (baseUIForms != null) return;
        Debug.Log("关闭其他窗体");
        //正在显示ui窗体缓存 与栈缓存集合里所有的窗体进行隐藏处理
        List<TurnUIShiny> ShowUIFormsList = new List<TurnUIShiny>(_WaxBizarreShunUIShiny.Values);
        foreach (TurnUIShiny baseUIFormsItem in ShowUIFormsList)
        {
        ...
        List<TurnUIShiny> CurrentUIFormsList = new List<TurnUIShiny>(_BedBizarreUIShiny);
        foreach (TurnUIShiny baseUIFormsItem in CurrentUIFormsList)
        {
    ...
    private void AprilUIShinyStandAcidXenon(string uiFormName, object uiFormParams)
    {
        ...
        foreach (TurnUIShiny uiforms in _WaxBizarreShunUIShiny.Values)
        {
    ...
    private void FeatUIShinyHeedStandWedShunApply(string strUIFormsName)
    {
        ...
        //foreach (TurnUIShiny baseUIFormsItem in _DicCurrentShowUIForms.Values)
        //{
        //    baseUIFormsItem.Redisplay();
        //}
        //foreach (TurnUIShiny baseUIFormsItem in _StaCurrentUIForms)
        //{
        //    baseUIFormsItem.Redisplay();
        //}
    }
    ...
    private void PushUIShiny(string strUIFormsName, object uiFormParams)
    {
        ...
        {
            TurnUIShiny topUIForms = _BedBizarreUIShiny.Peek();
            topUIForms.Occupy();
    ...
    private void BagUIShiny()
    {
        ...
        {
            //出栈逻辑
            TurnUIShiny topUIForms = _BedBizarreUIShiny.Pop();
            //出栈的窗体进行隐藏
            topUIForms.Hidding(() => {
                //出栈窗体的下一个窗体逻辑
                TurnUIShiny nextUIForms = _BedBizarreUIShiny.Peek();
                //下一个窗体重新显示处理
    ...
    {
        IExciteImagery configMgr = new ExciteImageryOrYarn(SysPurely.SYS_PATH_UIFORMS_CONFIG_INFO);
    ...
    /// </summary>
    /// <returns></returns>
    public Stack<TurnUIShiny> PutBizarreLiftBloom()
    {
    ...
    public List<GameObject> PutDisplayBeacon(bool containNormal = false)
    {
        ...
        List<TurnUIShiny> allUIFormsList = new List<TurnUIShiny>(_WaxALLUIShiny.Values);
        foreach(TurnUIShiny panel in allUIFormsList)
        {
