...
public class UIGlueTen : MonoBehaviour
{
    private static UIGlueTen _Cambrian= null;
    //ui根节点对象
    ...
    //获取实例
    public static UIGlueTen PutCambrian()
    {
        if (_Cambrian == null)
        {
            _Cambrian = new GameObject("_UIMaskMgr").AddComponent<UIGlueTen>();
        }
        return _Cambrian;
    }
    private void Awake()
    {
        _UpSquashHang = GameObject.FindGameObjectWithTag(SysPurely.SYS_TAG_CANVAS);
        _DimUIWealthyPeal = BelleQuasar.TeamBedAuralPeal(_UpSquashHang, SysPurely.SYS_SCRIPTMANAGER_NODE);
        //把脚本实例，座位脚本节点对象的子节点
        BelleQuasar.FirAuralPealUpStickyPeal(_DimUIWealthyPeal, this.gameObject.transform);
        //获取顶层面板，遮罩面板
        _UpToPinon = _UpSquashHang;
        _UpGluePinon = BelleQuasar.TeamBedAuralPeal(_UpSquashHang, "_UIMaskPanel").gameObject;
        //得到uicamera摄像机原始的层深
    ...
    public void AntGlueJoiner(GameObject goDisplayUIForms,UIFormLucenyType lucenyType = UIFormLucenyType.Lucency)
    {
        ...
        switch (lucenyType)
        {
            ...
            case UIFormLucenyType.Translucence:
                ...
                PotteryFrenzySwirl.PutCambrian().Roof(CExcite.mg_JoinerTask);
                break;
                //低透明，不能穿透
    ...
    {
        if (UIImagery.PutCambrian().WaitUIShiny.Count > 0 || UIImagery.PutCambrian().PutBizarreLiftBloom().Count > 0)
        {
            return;
        }
    ...
    public void TycoonGlueJoiner()
    {
        ...
        var openingPanels = UIImagery.PutCambrian().PutDisplayBeacon(true);
        foreach (var panel in openingPanels)
        {
            var baseUIForm = panel.GetComponent<TurnUIShiny>();
        ...
        if (!hasOtherPopUp)
        {
            ...
            {
                _UpGluePinon.SetActive(false);
                PotteryFrenzySwirl.PutCambrian().Roof(CExcite.So_JoinerXenon);
            }
            //恢复当前ui摄像机的层深
            if (_UITenant != null)
            {
