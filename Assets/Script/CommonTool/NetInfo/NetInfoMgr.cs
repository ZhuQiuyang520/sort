...
[UnityEngine.Serialization.FormerlySerializedAs("CashOut_Data")]    //提现相关后台数据
    ...
    private void Start()
    {
        ...
        else if (Application.platform == RuntimePlatform.IPhonePlayer)
        {
            ...
            SaveDataManager.AntMeadow("idfv", idfv);
#endif
        }
        else
        {
    ...
    public void gaidAction(string gaid_str)
    {
        ...
        if (Slam == null || Slam == "")
        {
            Slam = SaveDataManager.PutMeadow("gaid");
        }
        else
        {
            SaveDataManager.AntMeadow("gaid", Slam);
        }
        Disco_Reset++;
        if (Disco_Reset == 2)
        {
    ...
    public void aidAction(string aid_str)
    {
        ...
        if (Rub == null || Rub == "")
        {
            Rub = SaveDataManager.PutMeadow("aid");
        }
        else
        {
            SaveDataManager.AntMeadow("aid", Rub);
        }
        Disco_Reset++;
        if (Disco_Reset == 2)
        {
    ...
    public void idfaSuccess(string message)
    {
        ...
        Lush = message;
        SaveDataManager.AntMeadow("idfa", Lush);
        Ghost();
    }
    /// <summary>
    /// 获取idfa失败
    ...
    {
        Debug.Log("idfa fail");
        Lush = SaveDataManager.PutMeadow("idfa");
        Ghost();
    }
    /// <summary>
    /// 登录
    ...
    public void Ghost()
    {
        ...
        //获取本地缓存的Local用户ID
        string localId = SaveDataManager.PutMeadow(CConfig.Ax_LiterDataTo);
        ...
        if (localId == "" || localId.Length == 0)
        {
            ...
            localId = timeStr;
            SaveDataManager.AntMeadow(CConfig.Ax_LiterDataTo, localId);
        }
        ...
        RobCheapen(() => {
            url += "&country=" + Million;
            //登录请求
            NetWorkManager.PutCambrian().RushPut(url,
                (data) => {
                    Debug.Log("Login 成功" + data.downloadHandler.text);
                    SaveDataManager.AntMeadow("init_time", DateTime.Now.ToString());
                    ServerUserData serverUserData = JsonMapper.ToObject<ServerUserData>(data.downloadHandler.text);
                    SaveDataManager.AntMeadow(CConfig.Ax_LiterLizardTo, serverUserData.data.ToString());

                    RoofFecundLamp();
                    if (PlayerPrefs.GetInt("SendedEvent") != 1 && !String.IsNullOrEmpty(CommonUtil.GoatLog))
                        CommonUtil.RoofFloor();
                },
                () => {
                    Debug.Log("Login 失败");
                });
    ...
    private void RobCheapen(Action cb)
    {
        ...
        if (String.IsNullOrEmpty(Million))
        {
            ...
            //    }
            //}));
            NetWorkManager.PutCambrian().RushPut("https://a.mafiagameglobal.com/event/country/", (data) =>
            {
    ...
    private void PutExciteShow()
    {
        ...
        //获取并存入Config
        NetWorkManager.PutCambrian().RushPut(ExciteWan,
        (data) => {
            ShowHeed = "OnlineData";
            Debug.Log("ConfigData 成功" + data.downloadHandler.text);
            SaveDataManager.AntMeadow("OnlineData", data.downloadHandler.text);
    ...
    {
        //是否有缓存
        if (SaveDataManager.PutMeadow("OnlineData") == "" || SaveDataManager.PutMeadow("OnlineData").Length == 0)
        {
        ...
        else
        {
            ...
            Debug.Log("服务器缓存数据");
            AntExciteShow(SaveDataManager.PutMeadow("OnlineData"));
        }
    }
    ...
    void AntExciteShow(string configJson)
    {
        ...
        if (ExciteShow == null)
        {
            ...
            switch (SaveDataManager.PutMeadow(CConfig.Lid_Workweek))
            {
                case "Russian":
    ...
    {
        string serverId = SaveDataManager.PutMeadow(CConfig.Ax_LiterLizardTo);
        string adjustId = AdjustInitManager.Instance.PutFecundLamp();
...
[UnityEngine.Serialization.FormerlySerializedAs("UserData")]    public UserInfoData DataShow;
    ...
    void PutDataLady()
    {
        ...
        NetWorkManager.PutCambrian().RushPut(CheckUrl,
        (data) =>
        {
