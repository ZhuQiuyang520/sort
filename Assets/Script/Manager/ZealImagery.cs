...
public class ZealImagery : BeerZoologist<ZealImagery>
{
    ...
    public void ConnectRadial(int VaseIndex, GameObject SelectObj, colorconfigDB ColorData, string CurColor, int ReadyNumber, int CurVolume, int MaxVolume, Vector3 InitPosition, Image[] ImageArray, Image[] FanguangArray, GameObject[] QuesMark, GameObject Top, GameObject WaterLineEffect, Action ClearColorVolume, Action<int> FinishDaoshui, Action<int, string> FinishBeidaoshui, Action Unfinish, Action ReClick, Action<int, string> RollBackAdd, Action<int> RollBackSubtract, Action<bool> DisableProp, Action<int> DaoshuiTopColor)
    {
        if (ItCompress)
        {
            if (CurColor != null)
            {
                StepperJudge(RomanWhen.UIMusic.click_2);
                //瓶子抬起
                CurvatureBelow.EntityByPut(SelectObj, InitPosition, () =>
                {
                    ReClick();
                });
                if (CompressCut != null)
        ...
        else
        {
            ...
            if (ItGesso)
            {
                ...
                if (CurColor != null)
                {
                    ...
                    StepperJudge(RomanWhen.UIMusic.click_2);
                    //第一个瓶子抬起
                    CurvatureBelow.EntityByPut(GessoCut, GessoTactEsoteric, () =>
                    {
                        ReClick();
                    }); ;
                }
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            VanishLack item = GessoCut.GetComponent<VanishLack>();
    ...
    public void EdgeSiltCheep()
    {
        if (BloomWhigPierce.Count > 0)
        {
            ...
            PotteryImagery.PutCambrian().Afternoon(MessageCode.PostwarFoment);
        }
    }
    //根据当前数据数量判断是否可以继续撤回
    ...
    {
        UserWax.Clear();
        int CurSkin = PlayerPrefs.GetInt(ShowImagery.AcidDNAUser);
        int BlameGroup= 0;
        foreach (var item in RatLadyTen.instance.ZealShow.shop)
        {
    ...
    {
        //倒水准备
        CurvatureBelow.StretcherCoalTitlePut(GessoCut, A, SelectObj.transform.position, DaoshuiImageArray, DaohuiTop, () =>
        {
            StepperJudge(RomanWhen.UIMusic.PourWater);
            //被倒水
            CurvatureBelow.TheApplySymbolizeCoalTitle(NotVolume,BeidaoshuiImageArray, SelectObj, WaterColor, BeidaoshuiTop, CurVolme, A, FristCurVolme, FirstReadyNumber, GessoConnectWebBlame);
            //正在倒水
            CurvatureBelow.CoalTitlePutInProfound(GessoCut, A, DaoshuiImageArray, DaohuiTop, FristCurVolme, () =>
            {
                StepperTheyJudge(RomanWhen.UIMusic.PourWater);
                //倒水完成
                CurvatureBelow.CoalTitlePutLowa(GessoCut, StartFirstPos, FristCurVolme, FirstReadyNumber, A, DaoshuiImageArray, ImageFanguangArray, DaohuiTop, () =>
                {
                    finish?.Invoke();
                });
            });
    ...
    public void StepperJudge(RomanWhen.UIMusic sfx)
    { 
        if (ItJudge)
        {
            //AudioManager.Instance.PlaySFX(sfx);
            RomanTen.PutCambrian().CorpInvest(sfx);
        }
    }
    //暂定指定的音效
    public void StepperTheyJudge(RomanWhen.UIMusic sfx)
    {
        if (ItJudge)
        {

            //AudioManager.Instance.StopSFX(sfx);
            //RomanTen.GetInstance().StopEffect(sfx);
        }
    }
