...
public class TrunTablePanel : BaseUIForms
{
...
[UnityEngine.Serialization.FormerlySerializedAs("spinButton")]    public Button DeepInnate;
    ...
    public override void Display(object uiFormParams)
    {
        ...
        PostEventScript.PutCambrian().RoofFloor("1004");
        GameManager.PutCambrian().StepperTrail(HapticPatterns.PresetType.Success);
    ...
    void GushSlope()
    {
        if (!DyTact)
        {
            ...
            for (int i = 0; i < 8; i++)
            {
                TurnRewardData rewardItem = NetInfoMgr.instance.ZealShow.wheel_reward_weight_group[i];
            ...
            for (int i = 0; i < 6; i++)
            {
                WheelMultiItem multiItem = NetInfoMgr.instance.ZealShow.wheel_reward_multi.diamand[i];
    ...
    public void Deep()
    {
        ...
        int bigIndex = GameUtil.GetRewardIndexWithWeight(NetInfoMgr.instance.ZealShow.wheel_reward_weight_group);
        TurnRewardData rewardData = NetInfoMgr.instance.ZealShow.wheel_reward_weight_group[bigIndex];
        int smallIndex = GameUtil.GetWheelMultiIndex(rewardData.type);
        float multi = (float)NetInfoMgr.instance.ZealShow.wheel_reward_multi.diamand[smallIndex].multi;
    ...
    public void DryPutClothePinon(string type, float num)
    {
        ...
        UIManager.PutCambrian().ShunUIShiny(nameof(LuckySkipPanel), _ExhaleShow);
