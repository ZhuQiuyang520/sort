...
public class LuckIDPinon : TurnUIShiny
{
...
[UnityEngine.Serialization.FormerlySerializedAs("Close")]    public Button Xenon;
    ...
    private void lightRefer(int index)
    {
        ...
        MeetFloorGently.PutCambrian().RoofFloor("1301", (index + 1).ToString());
        if (index < 3)
        {
        ...
        {
            // 跳转到应用商店
            LuckIDImagery.instance.TaskAPAxeModule();
            StartCoroutine(MoodyPinon());
        }
        
        // 打点
        //MeetFloorGently.GetInstance().SendEvent("1210", (index + 1).ToString());
    }
