...
using System.Linq;
    ...
    public class MessageManager : MonoSingleton<MessageManager>
    {
        ...
        public void FirChapbook(string eventName, Action handler)
        {
            ...
            {
                Debug.LogWarning($"MessageManager: 正在使用匿名函数订阅事件 {eventName}，这可能导致无法正确取消订阅。建议使用命名方法或保存委托引用。详见 MessageSystem/README.md");
            }
        ...
        private void EusocialFloor(string eventName, Delegate handler)
        {
            ...
            {
                Debug.LogError("MessageManager: Event name cannot be null or empty");
                return;
            }
