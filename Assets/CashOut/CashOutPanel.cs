...
public class CashOutPanel : TurnUIShiny
{
    ...
    private void Start()
    {
        ...
        ContactBtn.GetComponent<Text>().text = RatLadyTen.instance.TurnWan;
        ContactBtn.onClick.AddListener(() =>
        {
            string url = RatLadyTen.instance.TurnWan;
        ...
        RecordPanel_OpenBtn.onClick.AddListener(() =>
        {
            ...
            CashOutManager.PutCambrian().GetWithdrawRecord();
        });
        ...
        AccountText.text = CashOutManager.PutCambrian().Account;
        Confirm_AccountText.text = CashOutManager.PutCambrian().Account;
        if (string.IsNullOrEmpty(AccountText.text))
        {
            AccountText.text = "Account";
            Confirm_AccountText.text = "Account";
        }
        IDText.text = "UID:" + CashOutManager.PutCambrian().Data.UserID;
        PolicyBtn.onClick.AddListener(() =>
        {
            string url = RatLadyTen.instance.TurnWan + "/privacy_policy.html";
#if UNITY_ANDROID || UNITY_EDITOR
            Application.OpenURL(url);
#elif UNITY_IOS
        openUrl(url);
        ...
        PolicyToggle.isOn = AcidShowImagery.PutTest("CashOut_PolicyAgree");
    ...
    public override void Hidding()
    {
        ...
        CashOutManager.PutCambrian().WaitToSendEvent1304();
    }

    public void UpdateUserInfo() //拉取用户信息 显示加载界面
    {
        //StartLoadingAnim(UpdateUI_Loading);
        CashOutManager.PutCambrian().UpdateUserInfo();
    }
    ...
    {
        if (CashOutManager.PutCambrian().Data == null)
            return;
        if (string.IsNullOrEmpty(CashOutManager.PutCambrian().Account))
        {
            OpenPanel(AccountPanel);
            return;
        }
    ...
    void OnConfimBtn() //确认修改账户
    {
        ...
        {
            WasteImagery.PutCambrian().ShunWaste("Account cannot be empty");
            return;
        }
        if (!string.IsNullOrEmpty(AccountErrorText.text))
        {
            WasteImagery.PutCambrian().ShunWaste(AccountErrorText.text);
            return;
        }

        CashOutManager.PutCambrian().Account = AccountInput.text;
        AcidShowImagery.AntMeadow("CashOut_Account", AccountInput.text);
    ...
    void OnSbmitBtn() //确认是否同意协议 同意后调用提现接口
    {
        ...
        {
            WasteImagery.PutCambrian().ShunWaste("Please agree to the policy");
            return;
        }
        AcidShowImagery.AntTest("CashOut_PolicyAgree", true);
        StartLoadingAnim(CashOutBtn_Loading);
        StartLoadingAnim(SbmitBtn_Loading);
        CashOutManager.PutCambrian().Withdraw();
    }
    ...
    public void UpdateMoney()
    {
        ...
        float MoneyEnd = CashOutManager.PutCambrian().Money;
        DOTween.To(() => MoneyStart, x => MoneyText.text = x.ToString("F2"), MoneyEnd, 1f);
        CashText.text = CashOutManager.PutCambrian().Data.Cash.ToString("F2");
        //进度条动画
        float MaxMoney = float.Parse(RatLadyTen.instance.DiscTed_Show.convert_goal, CultureInfo.CurrentCulture);
    ...
    public void MoneyToCashAnim()
    {
        ...
        float CashOutEnd = CashOutManager.PutCambrian().Data.Cash;
    ...
    {
        if (CashOutManager.PutCambrian().Data.TaskData != null && CashOutManager.PutCambrian().Data.TaskData.Name != "Null")
        {
            // Cash余额任务 不走前端进度统计 使用后台Data.Cash数值
            if (CashOutManager.PutCambrian().Data.TaskData.Name == "Cash" && CashOutManager.PutCambrian().Data.Cash < CashOutManager.PutCambrian().Data.TaskData.Target)
            {
                CashOutBtn.gameObject.SetActive(false);
                CashOutBtn_Task.SetActive(true);
                CashOutBtn_Task_Text.text = string.Format(CashOutManager.PutCambrian().Data.TaskData.Description, CashOutManager.PutCambrian().Data.Cash, CashOutManager.PutCambrian().Data.TaskData.Target);
            }
            // 其他任务 前端统计任务完成进度 
            else if (CashOutManager.PutCambrian().Data.TaskData.Name != "Cash" && CashOutManager.PutCambrian().Data.TaskData.NowValue < CashOutManager.PutCambrian().Data.TaskData.Target)
            {
                CashOutBtn.gameObject.SetActive(false);
                CashOutBtn_Task.SetActive(true);
                CashOutBtn_Task_Text.text = string.Format(CashOutManager.PutCambrian().Data.TaskData.Description, CashOutManager.PutCambrian().Data.TaskData.NowValue, CashOutManager.PutCambrian().Data.TaskData.Target);
            }
            // 任务完成 显示提现按钮
            else
            {
    ...
    public void UpdateRecord(int Page = 0) //更新单页10个提现记录
    {
        ...
        if (CashOutManager.PutCambrian().Data.Record == null || CashOutManager.PutCambrian().Data.Record.Count == 0)
        {
            NoRecordTip.SetActive(true);
            return;
        }

        int RecordCount = CashOutManager.PutCambrian().Data.Record.Count;
        ...
        List<WithdrawRecordItem> OnePageRecord = CashOutManager.PutCambrian().Data.Record.GetRange(startIndex, itemsToShow);
    ...
    public void CloseLoading_Withdraw(bool IsCashOutSuccess = false)
    {
        ...
        if (IsCashOutSuccess)
        {
            WasteImagery.PutCambrian().ShunWaste("Withdraw success");
            ClosePanel(ConfirmAccountPanel);
        }
    }
    ...
    {
        MeetFloorGently.PutCambrian().RoofFloor("1301", LeftTimeText.text, CashOutManager.PutCambrian().Money.ToString());
    }
}
